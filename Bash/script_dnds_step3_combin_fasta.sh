...
#Pareil, c'est facultatif parce que si on calcul le dnds avec le package seqinr il faut des fichiers combinés, si on le calcul avec le package ape, il faut des fichiers séparés
...
#Dossiers à éviter
DIR_A_EVITER=("ABG42891.1_119864_122032" "AFD97201.1_60044_60598")
#ABG42891.1_119864_122032
#AFD97201.1_60044_60598
#"ORF_33" "ORF_40" "ORF_41" "ORF_64"
...
for DIR in "$INPUT_DIR"/*/ ; do
    ...
    if [[ " $DIR_A_EVITER " =~ " $DIR_NAME " ]]; then
        echo "Ignorer $DIR_NAME"
        continue
    fi
    #else

    for SAMPLE_FILE in "$DIR"*.fasta; do
        echo "SAMPLE_FILE :  $SAMPLE_FILE"

        #vérifie si le fichier de référence existe
        REF_FILE=(${DIR}/*_gen_ref.fasta) #c'est le globs qui fait qu'on peut ecrire comme ça?
